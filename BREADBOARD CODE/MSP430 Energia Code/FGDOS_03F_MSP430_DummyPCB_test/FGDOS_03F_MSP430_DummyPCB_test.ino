...
// 08/04: Adapting it to FGD_03F with the MSP430
   ...
   05/05: Added WCK initialization and reset

   23/05: Removed WCK implementation. WCK signal supplied externally from Arduino. 
          Added code to set FRAM CS pin HIGH

   31/05:Disabled automatic recharging for the time being in fgdos_init()
...
#include <SPI.h>
//#include <msp430.h>                                                                               // $$
#include <driverlib.h>                                                                              // $$ for clock signal
...
/*
  // Test to see that RS485 transceiver O/P stops when the ~RE and DE pin is set LOW
  GPIO_setAsOutputPin(GPIO_PORT_P2, GPIO_PIN6);
...
void loop() {
  ...
  temperature = read_reg(SS1,TEMP);                                                       // temperature = read_reg(SS2,TEMP) - 87;
  ...
  // read the temperature, recharge count and frequency registers
  temperature = read_reg(SS2,TEMP);                                                       // temperature = read_reg(SS2,TEMP) - 87;
