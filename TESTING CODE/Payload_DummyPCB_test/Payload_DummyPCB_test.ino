...
/*
// 08/04: Adapting it to FGD_03F with the MSP430
   to do: change PWM clock signal implementation from Arduino to MSP430

   21/04: Trying to import to energia + some changes to read from both sensors now
        : Also trying to find which pin numbers to use for the MCU

   26/04: Changed pin definitions header file and added new pins 

   28/04: Troubleshooting WCK signal generation 

   04/05: Writing the code for Standalone functional Testing and Interface testing of Payload PCB

   05/05: Added extra variables to separately store Sensor 1 and 2 readout values

   10/05: Added functions to enable and disable RS485 transceiver transmit, to read data over Data Bus and send data over Data Bus

   11/05: Testing _delay_cycles()

   12/05: implementing Hyperion protocol temporarily

   13/05: Functional test of Payload Dummy PCB

   16/05: Testing DataBus Comms with Arduino

   23/05: Code added to set FRAM CS pin high to prevent SPI pollution

   31/05: Disabled automatic recharging in fgdos_init() and printing of "recharge in progress...BREAK"

   02/06: DataBus Comms and commands test with Arduino and LaunchPad, with level shifter only

   09/06: started adding crc check function
...
*/
  ...
  // setting NSTBY pins high using driverlib GPIO function (for LaunchPad and Breadboard B Setup)
...
//  GPIO_setOutputHighOnPin(GPIO_PORT_P3, GPIO_PIN6);
  ...
  SPI.beginTransaction(SPISettings(5000000, MSBFIRST, SPI_MODE0));                                                          //$$          30000
  SPI.begin();
