...
/*
 * 
 * 13/07: Script created to do an Interface Test between Radiation Payload MAIN Board and Daughterboard
 *        Added CS pins for external FGDOS 
 *        Added NSTBY pins for external FGDOS 
 *        Added WCK_2 signal for external FGDOS 
 *        Added variables for external FGDOS (_3 and _4) - sen_freq, ref_freq, temp, recharge_count
 *        Added functions for external FGDOS - external_clock_set and external_clock_reset (check if its needed) - not needed since both use SMCLK as the source
 *        Added serial prints of sensor readout for external FGDOS (_3 and _4)
 *        NIRQ_1 and NIRQ_2 are not used currently for either the internal or external FGDOS
 *        
 *        
 *        USEFUL RESOURCES:
...
#define NSTBY_3 33                    // P3.1  $$
#define NSTBY_4 34                    // P3.2  $$

// Window clock signal pins
#define WCK_1 8                       // P3.4
#define WCK_2 29                      // PJ.0 
//float clock_check_1 = 0;                        // to check clock signal value
//float clock_check_2 = 0;                        // to check clock signal value
...
unsigned long int sens_freq_3 = 0;
unsigned long int sens_freq_4 = 0;

unsigned long int ref_freq_1 = 0;
unsigned long int ref_freq_2 = 0;
unsigned long int ref_freq_3 = 0;
unsigned long int ref_freq_4 = 0;
...
int recharge_count_3 = 0;
int recharge_count_4 = 0;
...
void setup() {
  ...
  pinMode(WCK_2, OUTPUT);                                                                                                   // 
  PJSEL0 |= 0x01 ;   //Set PJSEL0.0 to 1 to select SMCLK   $$ 
  PJSEL1 |= 0x00 ;   //Set PJSEL1.0 to 0 to select SMCLK   $$ 
...
/*
  ...
  pinMode(NSTBY_3, OUTPUT);
  digitalWrite(NSTBY_3, HIGH);
  pinMode(NSTBY_4, OUTPUT);
  digitalWrite(NSTBY_4, HIGH);
*/
  //Comment the 8 lines below if using the LaunchPad and Breadboard B Setup                                                                 $$
  ...
  // External FGDOS NSTBY pins
  GPIO_setAsOutputPin(GPIO_PORT_P3, GPIO_PIN1);                         // $$  P3.1
  GPIO_setOutputHighOnPin(GPIO_PORT_P3, GPIO_PIN1);                     // $$
  GPIO_setAsOutputPin(GPIO_PORT_P3, GPIO_PIN2);                         // $$  P3.2
...
//  Serial.println("NSTBY pins are set HIGH");
  ...
  // Set External FGDOS CS pins to output mode and high
  pinMode(SS3, OUTPUT); //chip enable, active low
  pinMode(SS4, OUTPUT);
...
void loop() {
  ...
  // read the temperature, recharge count and frequency registers
  temperature_3 = read_reg(SS3,TEMP);                                                       // temperature = read_reg(SS2,TEMP) - 87;
  recharge_count_3 = read_reg(SS3,RECHARGE_COUNT);
  recharge_count_3 = recharge_count_3 & 0x0F;

  // collect freq from sensor 1 
  collect_freq(SS3,&sens_freq_3,&ref_freq_3);

  
  // Sensor 4           $$
  
  // read the temperature, recharge count and frequency registers
  temperature_4 = read_reg(SS4,TEMP);                                                       
  recharge_count_4 = read_reg(SS4,RECHARGE_COUNT);
  recharge_count_4 = recharge_count_4 & 0x0F;

  // collect freq from sensor 2 
  collect_freq(SS4,&sens_freq_4,&ref_freq_4);
  ...
  //Print Sensor 3 & 4 measurments to Serial Monitor                              $$
  print_meas_short(temperature_3,sens_freq_3,ref_freq_3,recharge_count_3,3);
  print_meas_short(temperature_4,sens_freq_4,ref_freq_4,recharge_count_4,4);
