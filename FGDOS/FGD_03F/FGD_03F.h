/* 
*  TO DO:
* - which TDIV to use in function of pulses and sens
* - 
 */
...
#define FGD_03F_h
  ...
  // nirq setting to push-pull (nirqoc bit 0) or open collector, measurement window to count clocks (engate bit 0)
  // edirt bit to 1
  ...
  // Cx default, 4x to disable auto recharge (called active and passive but is confusing, I know...)
    ...
    // the recharge voltage of 011 (16V) seems to be good, otherwise to fast and overshoots in recharging
  ...
  void collect_data(byte sensor,int *temperature, int*recharge_count, unsigned long int *sens_freq, unsigned long int *ref_freq, float window_factor);
  ...
  void set_command(char command, unsigned long int *sens_freq_1, unsigned long int *sens_freq_2, unsigned long int *target_freq,
                  ...
                  int *temperature_1, int *temperature_2, int *recharge_count_1, int *recharge_count_2, bool flag_isr);
