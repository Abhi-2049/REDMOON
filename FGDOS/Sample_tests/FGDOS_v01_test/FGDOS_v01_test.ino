// AS - script from Github, used for degugging with some changes

/*
 * Changes wrt v00
 * - add recharge functionality and perform test loop
 * - read recharge reg first to see if recharge is going on
 * - streamlined the code
 * - timeout of reading registers went off, added have_sens bools to prevent this
 */
//-----------------------------------------------------------------------------------------------------------------------
#include <SPI.h>
...
#define THRESHOLD_FREQ 30000 // these values are not exact! Since only 8 MSBs are used                                          // 30000
#define TARGET_FREQ 45000                                                                                                       // 45000
#define CK_FREQ 31250.0f // depends on the settings of the PWM                                                                     // 31250
#define WINDOW_PULSES 8192 // depends on window register settings!                                                              // 8192.0f
...
bool collect_freq(byte sensor, long unsigned int *sens_freq, long unsigned int *ref_freq);                                        // $ changed to long unsigned int
...
//  OCR2B = 31; // duty cycle = OCR2B+1 / OCR2A+1
  ...
  SPI.beginTransaction(SPISettings(5000000, MSBFIRST, SPI_MODE0));                                                            // 500000
  SPI.begin();
...
void loop() {
  ...
  Serial.print("recharge counter registry (BIN) : "); Serial.println(recharge_count,BIN);
  ...
  //trigger recharge
  if(i==2){
    Serial.print("recharge requested? : ");Serial.println((read_reg(SS2,RECHARGE_COUNT) & 0x40) >> 6);
    byte reg_threshold = round(45000/WINDOW_FACTOR/1023);
    long int thres = (reg_threshold << 10)*WINDOW_FACTOR;
    write_reg(SS2,THRESHOLD,reg_threshold);
    Serial.print("-threshold (8 MSB) : "); Serial.print(thres); Serial.print(" , "); Serial.println(reg_threshold,BIN);
    Serial.print("recharge requested? : ");Serial.println((read_reg(SS2,RECHARGE_COUNT) & 0x40) >> 6);
  ...
  if(j==9){
    // set recharge counter to 0 (disable requests, counter to 0, 'restart')

    Serial.println("-------------------- RESET --------------------");
    byte reg_threshold = round(30000/WINDOW_FACTOR/1023);
    long int thres = (reg_threshold << 10)*WINDOW_FACTOR;
    write_reg(SS2,THRESHOLD,reg_threshold);
    write_reg(SS2,RECHARGE_COUNT,0x00);
    j = 0;
    i = 0;
  }else{j++;}
...
bool collect_freq(byte sensor, long unsigned int *sens_freq, long unsigned int *ref_freq){                // $ changed arg 2 and 3 data type to long unsigned int
