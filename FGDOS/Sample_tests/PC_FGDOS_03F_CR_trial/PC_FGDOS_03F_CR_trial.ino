...
#include <SPI.h>
#include <FGD_03F_trial.h> //#include <FGD_03F.h>
...
void setup() {
  ...
  Serial.begin(250000); // higher for faster usart transfer, be careful not to lose data when transferring too fast11
  while(!Serial);
...
void loop() {
  ...
  } else if ((recharge_count_1 && 0x80) || (recharge_count_2 && 0x80)){
    if (recharge_count_1 && 0x80){collect_data(SS1, &temperature_1, &recharge_count_1, &sens_freq_1, &ref_freq_1, WINDOW_FACTOR);}
    if (recharge_count_2 && 0x80){collect_data(SS2, &temperature_2, &recharge_count_2, &sens_freq_2, &ref_freq_2, WINDOW_FACTOR);}
    wait(2.1/WINDOW_FACTOR*1000);
  }
