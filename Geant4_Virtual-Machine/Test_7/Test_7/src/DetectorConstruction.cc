...
/*
// Test_7: Simplified rover in vertical hole lava tube, multi-spectral, multi-directional source

Shape 1 - Main PCB
Shape 2 - Main FGDOS

Shape 3 - Chassis block
Shape 4 - Internal cavity
Shape 8 - Al chassis with cavity subtracted
...
Shape 9 - Lunar Regolith slab
Shape 10 - Cylindrical vertical hole
Shape 11 - Vertical hole lava tube (Shape 9 - Shape 10)
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // Envelope parameters
  //
  G4double env_sizeXY = 2000*cm, env_sizeZ = 2000*cm;												// $$  G4double env_sizeXY = 20*cm, env_sizeZ =30*cm;
...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Material definition for regolith Layer 1 - "regolith_L1"
  

	//Lunar Regolith (Mass fractions and density from Mckinney 2006) 
	//density = 1.76*g/cm3; 
	G4Material* regolith_L1 = new G4Material("regolith_L1", 1.76*g/cm3, 7); 
	regolith_L1->AddElement(nist->FindOrBuildElement("O"), 41.7*perCent);
	regolith_L1->AddElement(nist->FindOrBuildElement("Si"), 19.0*perCent);
	regolith_L1->AddElement(nist->FindOrBuildElement("Al"), 6.1*perCent);   
	regolith_L1->AddElement(nist->FindOrBuildElement("Ca"), 8.0*perCent);
	regolith_L1->AddElement(nist->FindOrBuildElement("Fe"), 14.0*perCent);
	regolith_L1->AddElement(nist->FindOrBuildElement("Mg"), 6.2*perCent);
	regolith_L1->AddElement(nist->FindOrBuildElement("Ti"), 5.0*perCent);



  //
  // Shape 9 - Lunar Regolith Slab - regolith_L1 - Box - 1000 cm X 1000 cm X 22 cm
  // position - 8.2 cm below MAIN PCB datum plane
  G4Material* shape9_mat = nist->FindOrBuildMaterial("regolith_L1");
  G4ThreeVector pos9 = G4ThreeVector(0, 0*cm, -19.2*cm);

  // Box shape
	G4double shape9_hx =  1000.0*cm;
  G4double shape9_hy =  1000.0*cm;
  G4double shape9_hz = 	1000.0*cm;  
	
	G4Box* solidShape9 =
    new G4Box("Shape9",                    //its name
        0.5*shape9_hx, 0.5*shape9_hy, 0.5*shape9_hz); //its size




  //
  // Shape 10 - Vertical hole - cavity - Cylinder - 250 cm X 500 cm
  // 
  G4ThreeVector pos10 = G4ThreeVector(0, 0*cm, 250*cm);

  // Box shape
	G4double shape10_r =  125.0*cm;
  G4double shape10_h =  500.0*cm;
  //G4double shape10_dz = 	100.0*cm;          // translation along z axis
  G4double shape10_phimin = 0.*deg, shape10_phimax = 360.*deg;
  G4RotationMatrix* rotm  = new G4RotationMatrix();
	 

	G4Tubs* solidShape10 = new G4Tubs("Shape10",0.*cm,shape10_r, 0.5*shape10_h, shape10_phimin, shape10_phimax); //its size
  ...
  //
  // Shape 11 - Verical hole lava tube - Solid formed by Boolean subtraction of shape 10 from shape 9
  // 

	G4SubtractionSolid* solidShape11 = 
  new G4SubtractionSolid("Shape11", solidShape9, solidShape10,rotm,pos10);    // cylinder rotation by 0 radian and translation by pos 10 

	//G4SubtractionSolid* solidShape11 = 
//  new G4SubtractionSolid("Shape11", solidShape9, solidShape10,rotm,pos10);    // cylinder rotation by 0 radian and translation by pos 10 

	// Logical volume - shape 11
  G4LogicalVolume* logicShape11 =
    new G4LogicalVolume(solidShape11,         //its solid
                        shape9_mat,          //its material
                        "Shape11");           //its name
	// Physical volume - shape 11
  new G4PVPlacement(0,                       //no rotation
                    pos9,                    //at position
...
///////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//
	//fScoringVolume = logicShape7;																			// Uncomment to set Daughterboard FGDOS as scoring volume
